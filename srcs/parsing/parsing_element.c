...
static int	new_cmd2(char *str, t_check *check, int *j, t_dt_elem **token)
{
	if (ft_strncmp(str + *j, "<<", 2) == 0 || ft_strncmp(str + *j, "<", 1) == 0
		|| ft_strncmp(str + *j, ">>", 2) == 0 || ft_strncmp(str + *j, ">",
			1) == 0)
	{
		if (tokens_redir(str + *j, token, check, j) == 0)
...
int	new_cmd(char *str, t_check *check, int *i, t_dt_elem **curr_block)
{
	int			j;
	t_dt_elem	*token;
	...
	while (str[j] && is_operator(str + j) == 0)
	{
		if (str[j] == '\"' || str[j] == '\'')
		{
			if (token_quotes(str + j, &token, check, str[j]) == 0)
				return (0);
		}
		else if (new_cmd2(str, check, &j, &token) == 0)
